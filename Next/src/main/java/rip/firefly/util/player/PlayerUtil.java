...
public class PlayerUtil {
    ...
    public static boolean hasBlocksNear(Player player) {
        for (Block block : BlockUtil.getSurroundingXZ(player.getLocation().getBlock())) {
    ...
    public static boolean hasBlocksNear(Location location) {
        for (Block block : BlockUtil.getSurroundingXZ(location.getBlock())) {
            if (block.getType().isSolid()) {
                return true;
            }
        }
    ...
    public static boolean isOnClimbable(Player player, int blocks) {
        ...
        if(System.currentTimeMillis() - DataManager.getData(player).getLastJoin() < 1000
                || DataManager.getData(player).getTeleportTicks() > 0
                || Bukkit.getPlayer(DataManager.getData(player).getUuid()).getLocation().getChunk() == null
                || !Bukkit.getPlayer(DataManager.getData(player).getUuid()).getLocation().getChunk().isLoaded()
        ) return true;
    ...
    public static boolean inInLilyPad(Player player) {
        Material m = player.getLocation().getBlock().getType();
        return m == Material.WATER_LILY;
    }

    public static boolean hasLilyPadsAround(Player player) {
        for (Block block : BlockUtil.getSurroundingXZ(player.getLocation().getBlock())) {
    ...
    public static boolean hasChestNear(Player player) {
        for (Block block : BlockUtil.getSurroundingXZ(player.getLocation().clone().subtract(0.0, 0.8, 0.0D).getBlock())) {
            if (block.getType().equals(Material.CHEST) || block.getType().equals(Material.TRAPPED_CHEST)) {
                return true;
            }
        }
        for (Block block : BlockUtil.getSurroundingXZ(player.getLocation().clone().subtract(0.0, 2.0, 0.0D).getBlock())) {
            if (block.getType().equals(Material.CHEST) || block.getType().equals(Material.TRAPPED_CHEST)) {
                return true;
            }
        }
    ...
    private static double[] getOffsetsOffCursor(Player player, LivingEntity entity) {
        ...
        double horizontalDistance = MathUtil.getHorizontalDistance(playerLoc, entityLoc);
    ...
    public static boolean isOnCarpet(Player player) {
        ...
        return (block.getType() == carpet) || (block.getRelative(BlockFace.NORTH).getType() == carpet)
                || (block.getRelative(BlockFace.SOUTH).getType() == carpet)
                || (block.getRelative(BlockFace.EAST).getType() == carpet)
                || (block.getRelative(BlockFace.WEST).getType() == carpet);
    }
    ...
    public static boolean isOnGround(Location loc) {
        ...
        Bukkit.getScheduler().callSyncMethod(FireFly.getInstance().getPlugin(), () -> {
            atomicBoolean.set(!isGround(loc.clone().add(0, -.1, 0).getBlock().getType())
    ...
    public static boolean groundBlocksAround(Player p) {
        Location loc = p.getLocation();

        boolean directlyAround = loc.clone().add(1, 0, 0).getBlock().getType().equals(Material.AIR) || loc.clone().add(-1, 0, 0).getBlock().getType().equals(Material.AIR) || loc.clone().add(0, 0, 1).getBlock().getType().equals(Material.AIR) || loc.clone().add(0, 0, -1).getBlock().getType().equals(Material.AIR);


        return directlyAround;
   //     loc.clone().add(-1, 0, 1);
...
//        loc.clone().add(1, 0, -1);
  ...
  //      loc.clone().add(1, 0, 1);
    ...
    public static boolean slabsNear(PlayerLocation loc, Player p) {
        boolean nearBlocks = false;
        for (Block bl : getSurrounding(loc.getBlock(p), true)) {
            if ((bl.getType().equals(Material.STEP)) || (bl.getType().equals(Material.DOUBLE_STEP))
        ...
        for (Block bl : getSurrounding(loc.getBlock(p), false)) {
            if ((bl.getType().equals(Material.STEP)) || (bl.getType().equals(Material.DOUBLE_STEP))
                    || (bl.getType().equals(Material.WOOD_DOUBLE_STEP)) || (bl.getType().equals(Material.WOOD_STEP))) {
                nearBlocks = true;
                break;
        ...
        if (isBlock(loc.getBlock(p).getRelative(BlockFace.DOWN), new Material[] { Material.STEP, Material.DOUBLE_STEP,
    ...
    public static double getHDistance(PlayerLocation from, PlayerLocation to) {
        return getDistance(to.getX(), to.getZ(), from.getX(), from.getZ());
    }
    ...
    public static boolean isOnIce(Location loc) {
            double diff = 0.3;
            return BlockUtil.isIce(loc.clone().add(0, 0D, 0).getBlock())
                    || BlockUtil.isIce(loc.clone().add(diff, 0D, 0).getBlock())
                    || BlockUtil.isIce(loc.clone().add(-diff, 0D, 0).getBlock())
                    || BlockUtil.isIce(loc.clone().add(0, 0D, diff).getBlock())
                    || BlockUtil.isIce(loc.clone().add(0, 0D, -diff).getBlock())
                    || BlockUtil.isIce(loc.clone().add(diff, 0D, diff).getBlock())
                    || BlockUtil.isIce(loc.clone().add(diff, 0D, -diff).getBlock())
                    || BlockUtil.isIce(loc.clone().add(-diff, 0D, diff).getBlock())
                    || BlockUtil.isIce(loc.clone().add(-diff, 0D, -diff).getBlock());
