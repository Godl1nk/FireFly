...
public class RayTrace {
    ...
    public Vector positionOfIntersection(SRayCBox collisionBox, double blocksAway, double accuracy) {
        List<Vector> positions = traverse(blocksAway, accuracy);
        for (Vector position : positions) {
    ...
    public Vector positionOfIntersection(SRayCBox collisionBox, double skip, double blocksAway, double accuracy) {
    ...
    //bounding blockbox instead of vector
    public boolean intersects(SRayCBox collisionBox, double blocksAway, double accuracy) {
        List<Vector> positions = traverse(blocksAway, accuracy);
        for (Vector position : positions) {
            if (intersects(position, collisionBox.min(), collisionBox.max())) {
                return true;
    ...
    //bounding blockbox instead of vector
    public boolean intersects(RBoundingBox collisionBox, double blocksAway, double accuracy) {
        List<Vector> positions = traverse(blocksAway, accuracy);
        for (Vector position : positions) {
    ...
    public boolean intersects(SRayCBox collisionBox, double skip, double blocksAway, double accuracy) {
        List<Vector> positions = traverse(blocksAway, accuracy);
        for (Vector position : positions) {
            if (intersects(position, collisionBox.min(), collisionBox.max())) {
                return true;
