...
public class PlayerUtils {
    ...
    public static boolean wasOnSlime(Player player) {
        PlayerData user = PluginManager.instance.getDataManager().getData(player);
    ...
    public static boolean isOnTrapDoor(Player player) {
        Block block = player.getLocation().getBlock();
        Material trapDoor = Material.TRAP_DOOR;

        return (block.getType() == trapDoor) || (block.getRelative(BlockFace.NORTH).getType() == trapDoor)
                || (block.getRelative(BlockFace.SOUTH).getType() == trapDoor)
                || (block.getRelative(BlockFace.EAST).getType() == trapDoor)
                || (block.getRelative(BlockFace.WEST).getType() == trapDoor);
    }
}
